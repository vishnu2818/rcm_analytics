...
def classify_ar_status(row, payment_status):
    ...
    if payment_status == "Negative balance":
        return "Negative Ins AR"
            ...
            (float(row.get('Balance amt', 0) or 0) == 0 and float(row.get('Charge amt', 0) or 0) == 0 and status in [
                'canceled', 'closed']) or
...
from django.http import HttpResponse
from .models import ExcelData
from .views import classify_payment_status, classify_ar_status, classify_claim_status
...
def test_display_data_verbose(request):
    ...
    try:
        ...
        return HttpResponse(f"Error retrieving queryset: {e}", status=500)
    ...
    for i in range(len(queryset)):
        try:
            row = queryset[i]  # Force ORM evaluation one-by-one
            d = row.data or {}

            ps = classify_payment_status(d)
            ars = classify_ar_status(d, ps)
            cs, _ = classify_claim_status(d, ps, ars)
            ...
            print(f"⚠️ OSError on row index {i}: {io_err}")
            continue
        except Exception as e:
            print(f"⚠️ General error on row index {i}: {e}")
            continue
